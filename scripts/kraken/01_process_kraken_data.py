#!/usr/bin/env python
# scripts/kraken/01_process_kraken_data.py

"""
Process and analyze Kraken2/Bracken microbial profiling data.

This script:
1. Loads Kraken2 reports and Bracken abundance files
...
Usage:
    python scripts/kraken/01_process_kraken_data.py [--config CONFIG_FILE]
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Process and analyze Kraken2/Bracken data')
    parser.add_argument('--config', type=str, default='config/analysis_parameters.yml',
                       help='Path to configuration file')
    parser.add_argument('--kreport-dir', type=str, 
                        default='~/Analysis/TrimmedMSSFiles/KrakenOutput/reports',
                       help='Path to directory containing Kraken report files')
    parser.add_argument('--bracken-dir', type=str, 
                        default='~/Analysis/TrimmedMSSFiles/BrackenOutput',
                       help='Path to directory containing Bracken abundance files')
    parser.add_argument('--output-dir', type=str, default='results/kraken_analysis',
...
def load_bracken_files(bracken_dir, taxonomic_level='S'):
    """
    Load Bracken abundance files from a directory and combine them into a single DataFrame.
    
    Parameters:
    -----------
    bracken_dir : str or Path
        Path to directory containing Bracken abundance files
    taxonomic_level : str
        Taxonomic level to use (D, P, C, O, F, G, S)
    ...
    try:
        ...
        bracken_dir = os.path.expanduser(bracken_dir)
        
        if not os.path.exists(bracken_dir):
            raise ValueError(f"Bracken directory not found: {bracken_dir}")
        
        # Find all Bracken abundance files for the specified taxonomic level
        file_pattern = f"*_{taxonomic_level}_abundance.txt"
        bracken_files = [os.path.join(bracken_dir, f) for f in os.listdir(bracken_dir) 
                        if f.endswith(f"_{taxonomic_level}_abundance.txt")]
        
        if not bracken_files:
            raise ValueError(f"No Bracken abundance files found for level {taxonomic_level} in {bracken_dir}")
            
        print(f"Found {len(bracken_files)} Bracken abundance files")
        ...
        for file_path in bracken_files:
            try:
                ...
                if 'name' not in df.columns or 'fraction_total_reads' not in df.columns:
                    print(f"Warning: {file_path} doesn't have expected columns, skipping")
                    continue
                
                # Extract taxon names and abundances
                taxa_abundances = dict(zip(df['name'], df['fraction_total_reads']))
                all_data[sample_name] = taxa_abundances
                
            except Exception as e:
                print(f"Error processing {file_path}: {str(e)}")
        
        if not all_data:
            raise ValueError("No valid Bracken abundance data found")
        
        # Create a DataFrame from the collected data
        abundance_df = pd.DataFrame(all_data)
        ...
        if abundance_df.shape[0] == 0 or abundance_df.shape[1] == 0:
            raise ValueError(f"Combined abundance data has {abundance_df.shape[0]} taxa and {abundance_df.shape[1]} samples")
        
        return abundance_df
        
    except Exception as e:
        print(f"Error loading Bracken files: {str(e)}")
        sys.exit(1)
        
        
def process_kraken_abundance(kreport_dir, bracken_dir, output_dir, taxonomic_level='S'):
    """
    Process Kraken reports and Bracken abundance files using kraken_tools.
    
    Parameters:
    -----------
    kreport_dir : str
        Path to directory containing Kraken report files
    bracken_dir : str
        Path to directory containing Bracken abundance files
    output_dir : str
    ...
    """
    if not kraken_tools_available:
        print("Warning: kraken_tools not available, using built-in function instead")
        return None
        
    try:
        # Expand user directories if needed
        kreport_dir = os.path.expanduser(kreport_dir)
        bracken_dir = os.path.expanduser(bracken_dir)
        ...
        # Look for Bracken abundance files
        bracken_files = {}
        sample_list = []
        
        # List files in bracken directory
        for filename in os.listdir(bracken_dir):
            if filename.endswith(f"_{taxonomic_level}_abundance.txt"):
                sample_id = filename.split("_")[0]
                bracken_files[sample_id] = os.path.join(bracken_dir, filename)
                sample_list.append(sample_id)
        
        if not bracken_files:
            print(f"No Bracken abundance files found for level {taxonomic_level}")
            return None
        ...
        if not abundance_file:
            print("Error: Could not produce an abundance file for analysis")
            return None
            
        print(f"Processed abundance file saved to {abundance_file}")
        return abundance_file
        
    except Exception as e:
        print(f"Error processing Kraken/Bracken files: {str(e)}")
        return None
...
def main():
    """Main function to process and analyze Kraken2/Bracken abundance data."""
    # Parse arguments
    args = parse_args()
    ...
    if os.path.exists(config_path):
        ...
        config = {
            'metadata': {
                'filename': args.metadata_file,
                'sample_id_column': args.sample_id_column,
                'group_variables': ['Severity', 'Symptoms'],
    ...
    # Process Kraken/Bracken data using kraken_tools if available
    abundance_file = None
    
    if kraken_tools_available:
        print(f"Processing Kraken reports and Bracken files...")
        abundance_file = process_kraken_abundance(
            args.kreport_dir,
            args.bracken_dir,
            output_dir,
    ...
    # If kraken_tools processing failed or not available, use built-in function
    if not abundance_file:
        print(f"Loading Bracken abundance data directly...")
        abundance_df = load_bracken_files(args.bracken_dir, args.taxonomic_level)
        
        # Save the combined abundance table
        abundance_file = output_dir / f'combined_bracken_{args.taxonomic_level.lower()}_abundance.tsv'
        abundance_df.to_csv(abundance_file, sep='\t')
        print(f"Combined abundance table saved to {abundance_file}")
    else:
        # Load the processed abundance file
    ...
    filtered_file = output_dir / f'filtered_kraken_{args.taxonomic_level.lower()}_abundance.tsv'
    filtered_abundance_df.to_csv(filtered_file, sep='\t')
    print(f"Filtered abundance table saved to {filtered_file}")
    
    # Load metadata
    metadata_file = args.metadata_file or config['metadata']['filename']
    if kraken_tools_available:
        # Use kraken_tools metadata loading function
        metadata_df = read_and_process_metadata(metadata_file)
    else:
    ...
    alpha_file = tables_dir / f'alpha_diversity_{args.taxonomic_level.lower()}.tsv'
    alpha_div.to_csv(alpha_file, sep='\t')
